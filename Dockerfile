...
# Copy the PDF file into the image
COPY document/Licitek.pdf ${LAMBDA_TASK_ROOT}/document/

# Install the specified packages
RUN pip install -r requirements.txt

# Copy function code
COPY RAG_main.py ${LAMBDA_TASK_ROOT}

# Set the permissions to make the file executable
RUN chmod +x RAG_main.py

# Set the CMD to your handler
CMD [ "RAG_main.lambda_handler" ]